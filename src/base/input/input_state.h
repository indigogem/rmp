...
namespace kmp::input
{
    ...
    public:
        KMP_FORCE_INLINE bool WasPressed() const { return m_state == State::Pressed; }
        KMP_FORCE_INLINE bool WasReleased() const { return m_state == State::Released; }
        KMP_FORCE_INLINE bool IsHeldDown() const { return m_state == State::Held; }

        KMP_FORCE_INLINE Seconds GetTimeHeld() const
        {
            KMP_ASSERT(IsHeldDown() || WasPressed());
            return m_holdTime;
        }
    ...
    private:
        ...
        KMP_FORCE_INLINE void SetState(State inState)
        {
            m_state = inState;
        ...
        KMP_FORCE_INLINE void ReflectFrom(Seconds deltaTime, Button const &sourceButton)
        {
    ...
    public:
        ...
        {
            KMP_ASSERT(buttonIdx < numButtons);
    ...
    protected:
        ...
        {
            kmp::memory::MemsetZero(m_buttons.data(), m_buttons.size() * sizeof(Button));
        }
        ...
        {
            Seconds const scaledDeltaTime = deltaTime * math::fmax(0.0f, timeScale);
            for (auto i = 0; i < numButtons; i++)
            {
    ...
    private:
        Array<Button, numButtons> m_buttons;
    };
}